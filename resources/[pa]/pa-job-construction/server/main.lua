...
local BASE_PAYOUT = 180
local ALLOWED_JOBS = { ['construction'] = true }
...
local function pickRoute(src)
    ...
    TriggerClientEvent('pa:job-construction:route', src, {
...
RegisterNetEvent('pa:job-construction:start', function()
    local src = source
    local ok, err = ensureJobActive(src)
    if not ok then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Construction', message = err, duration = 4500 })
        return
    end

    TriggerClientEvent('pa:job-construction:state', src, { active = true })
    pickRoute(src)
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Construction', message = 'Shift started. Follow your route.', duration = 3500 })
end)

RegisterNetEvent('pa:job-construction:stop', function()
    local src = source
    ActiveRoutes[tostring(src)] = nil
    TriggerClientEvent('pa:job-construction:state', src, { active = false })
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Construction', message = 'Shift ended.', duration = 3000 })
end)

RegisterNetEvent('pa:job-construction:complete', function(payload)
    local src = source
    payload = type(payload) == 'table' and payload or {}

    local route = ActiveRoutes[tostring(src)]
    if not route then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Construction', message = 'No active route.', duration = 3000 })
        return
    end
    ...
    if (now - last) < COOLDOWN_SECONDS then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Construction', message = 'Slow down, route cooldown active.', duration = 3000 })
    ...
    if not distanceOk then
        TriggerClientEvent('pa:ui:notify', src, { type = 'error', title = 'Construction', message = 'Route validation failed.', duration = 4000 })
    ...
    local reason = 'pa-job-construction:route_complete'

    local extraMessage = ''
    local extraMeta = {}

    if 'city_project_progress' == 'npc_tip' then
        local tip = math.random(10, 70)
        payout = payout + tip
        extraMeta.tip = tip
        extraMessage = (' + $%s NPC tip'):format(tip)
    elseif 'city_project_progress' == 'rare_find' then
    ...
    elseif 'city_project_progress' == 'city_project_progress' then
