...
local function getConfigByName(name)
    ...
    if not config then
        error(('Unknown shared config "%s". Valid names: items, jobs, factions, licenses, story_chapters, economy, businesses, housing, crime, crime_activities, strings.'):format(name), 2)
    end
