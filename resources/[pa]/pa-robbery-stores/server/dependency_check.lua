...
local REQUIRED_DEPENDENCIES = {
    { name = 'oxmysql', reason = 'Required before pa-robbery-stores can initialize safely.' },
    { name = 'ox_lib', reason = 'Required before pa-robbery-stores can initialize safely.' },
    { name = 'pma-voice', reason = 'Required before pa-robbery-stores can initialize safely.' },
    { name = 'ox_target', reason = 'Required before pa-robbery-stores can initialize safely.' },
    { name = 'pa-core', reason = 'pa-core is the only hard gameplay dependency and must start first.' },
    { name = 'pa-shared', reason = 'Required for tier balancing and unlock rules.' },
    { name = 'pa-crime', reason = 'Required for heat/case/evidence progression tracking.' },
    { name = 'pa-factions', reason = 'Required for faction-rank unlock checks and RP links.' },
    { name = 'pa-economy', reason = 'Required for dirty-money payouts.' },
    { name = 'pa-inventory', reason = 'Required for item-based tier gate checks.' },
    { name = 'pa-guard', reason = 'Required for rate limiting and distance validation.' },
    { name = 'pa-logging', reason = 'Required for suspicious behavior and payout logging.' },
}
