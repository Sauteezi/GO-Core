...
local BASE_PAYOUT = 170
local ALLOWED_JOBS = { ['tow'] = true }
...
local function pickRoute(src)
    ...
    TriggerClientEvent('pa:job-tow:route', src, {
...
RegisterNetEvent('pa:job-tow:start', function()
    local src = source
    local ok, err = ensureJobActive(src)
    if not ok then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Tow', message = err, duration = 4500 })
        return
    end

    TriggerClientEvent('pa:job-tow:state', src, { active = true })
    pickRoute(src)
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Tow', message = 'Shift started. Follow your route.', duration = 3500 })
end)

RegisterNetEvent('pa:job-tow:stop', function()
    local src = source
    ActiveRoutes[tostring(src)] = nil
    TriggerClientEvent('pa:job-tow:state', src, { active = false })
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Tow', message = 'Shift ended.', duration = 3000 })
end)

RegisterNetEvent('pa:job-tow:complete', function(payload)
    local src = source
    payload = type(payload) == 'table' and payload or {}

    local route = ActiveRoutes[tostring(src)]
    if not route then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Tow', message = 'No active route.', duration = 3000 })
        return
    end
    ...
    if (now - last) < COOLDOWN_SECONDS then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Tow', message = 'Slow down, route cooldown active.', duration = 3000 })
    ...
    if not distanceOk then
        TriggerClientEvent('pa:ui:notify', src, { type = 'error', title = 'Tow', message = 'Route validation failed.', duration = 4000 })
    ...
    local reason = 'pa-job-tow:route_complete'

    local extraMessage = ''
    local extraMeta = {}

    if 'impound_bonus' == 'npc_tip' then
        local tip = math.random(10, 70)
        payout = payout + tip
        extraMeta.tip = tip
        extraMessage = (' + $%s NPC tip'):format(tip)
    elseif 'impound_bonus' == 'rare_find' then
    ...
    elseif 'impound_bonus' == 'city_project_progress' then
