...
local BASE_PAYOUT = 165
local ALLOWED_JOBS = { ['taxi'] = true }
...
local function pickRoute(src)
    ...
    TriggerClientEvent('pa:job-taxi:route', src, {
...
RegisterNetEvent('pa:job-taxi:start', function()
    local src = source
    local ok, err = ensureJobActive(src)
    if not ok then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Taxi', message = err, duration = 4500 })
        return
    end

    TriggerClientEvent('pa:job-taxi:state', src, { active = true })
    pickRoute(src)
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Taxi', message = 'Shift started. Follow your route.', duration = 3500 })
end)

RegisterNetEvent('pa:job-taxi:stop', function()
    local src = source
    ActiveRoutes[tostring(src)] = nil
    TriggerClientEvent('pa:job-taxi:state', src, { active = false })
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Taxi', message = 'Shift ended.', duration = 3000 })
end)

RegisterNetEvent('pa:job-taxi:complete', function(payload)
    local src = source
    payload = type(payload) == 'table' and payload or {}

    local route = ActiveRoutes[tostring(src)]
    if not route then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Taxi', message = 'No active route.', duration = 3000 })
        return
    end
    ...
    if (now - last) < COOLDOWN_SECONDS then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Taxi', message = 'Slow down, route cooldown active.', duration = 3000 })
    ...
    if not distanceOk then
        TriggerClientEvent('pa:ui:notify', src, { type = 'error', title = 'Taxi', message = 'Route validation failed.', duration = 4000 })
    ...
    local reason = 'pa-job-taxi:route_complete'

    local extraMessage = ''
    local extraMeta = {}

    if 'npc_tip' == 'npc_tip' then
        local tip = math.random(10, 70)
        payout = payout + tip
        extraMeta.tip = tip
        extraMessage = (' + $%s NPC tip'):format(tip)
    elseif 'npc_tip' == 'rare_find' then
    ...
    elseif 'npc_tip' == 'city_project_progress' then
