...
local BASE_PAYOUT = 140
local ALLOWED_JOBS = { ['delivery'] = true }
...
local function pickRoute(src)
    ...
    TriggerClientEvent('pa:job-delivery:route', src, {
...
RegisterNetEvent('pa:job-delivery:start', function()
    local src = source
    local ok, err = ensureJobActive(src)
    if not ok then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Delivery', message = err, duration = 4500 })
        return
    end

    TriggerClientEvent('pa:job-delivery:state', src, { active = true })
    pickRoute(src)
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Delivery', message = 'Shift started. Follow your route.', duration = 3500 })
end)

RegisterNetEvent('pa:job-delivery:stop', function()
    local src = source
    ActiveRoutes[tostring(src)] = nil
    TriggerClientEvent('pa:job-delivery:state', src, { active = false })
    TriggerClientEvent('pa:ui:notify', src, { type = 'info', title = 'Delivery', message = 'Shift ended.', duration = 3000 })
end)

RegisterNetEvent('pa:job-delivery:complete', function(payload)
    local src = source
    payload = type(payload) == 'table' and payload or {}

    local route = ActiveRoutes[tostring(src)]
    if not route then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Delivery', message = 'No active route.', duration = 3000 })
        return
    end
    ...
    if (now - last) < COOLDOWN_SECONDS then
        TriggerClientEvent('pa:ui:notify', src, { type = 'warn', title = 'Delivery', message = 'Slow down, route cooldown active.', duration = 3000 })
    ...
    if not distanceOk then
        TriggerClientEvent('pa:ui:notify', src, { type = 'error', title = 'Delivery', message = 'Route validation failed.', duration = 4000 })
    ...
    local reason = 'pa-job-delivery:route_complete'

    local extraMessage = ''
    local extraMeta = {}

    if 'package_quality_bonus' == 'npc_tip' then
        local tip = math.random(10, 70)
        payout = payout + tip
        extraMeta.tip = tip
        extraMessage = (' + $%s NPC tip'):format(tip)
    elseif 'package_quality_bonus' == 'rare_find' then
    ...
    elseif 'package_quality_bonus' == 'city_project_progress' then
