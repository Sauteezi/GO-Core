...
local REQUIRED_DEPENDENCIES = {
    { name = 'oxmysql', reason = 'Required before pa-core can initialize safely.' },
    { name = 'ox_lib', reason = 'Required before pa-core can initialize safely.' },
    { name = 'pma-voice', reason = 'Required before pa-core can initialize safely.' },
    { name = 'ox_target', reason = 'Required before pa-core can initialize safely.' },
    { name = 'pa-shared', reason = 'Required before pa-core can initialize safely.' },
}
...
local function checkDependencies()
    ...
    if not isResourceStarted('pa-logging') then
        print(('^3[%s] Optional recommendation:^7 Start pa-logging after pa-core for structured traces.'):format(RESOURCE_NAME))
    end
    return true
end
